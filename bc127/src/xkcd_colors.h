#ifndef BC127_XKCD_COLORS_H
#define BC127_XKCD_COLORS_H

// =============================================================================
// xkcd_colors.h — XKCD colour name → RGB lookup table
// =============================================================================
//
// Subset of the 949 XKCD colour survey results.  Stored in PROGMEM to keep
// RAM free.  Names are lowercase; lookup strips spaces and lowercases input
// before comparing.
//
...
// The table is large — all entries live in flash via PROGMEM where the
// compiler supports it.  On ESP32 this is automatic for const globals.
static const XkcdColour XKCD_COLORS[] = {
    // ── Reds ────────────────────────────────────────────────────────────────
    { "red",                0xe50000 },
    { "dark red",           0x840000 },
    { "light red",          0xff474c },
    ...
    // ── Misc ────────────────────────────────────────────────────────────────
    { "warm",               0xfbb972 },  // warm (xkcd: warm brown)
    { "cool blue",          0x4984b8 },
    { "cornflower",         0x6a79f7 },
    { "steel",              0x738595 },
    { "dusk",               0x4e5481 },
...
static constexpr int NUM_XKCD_COLORS = sizeof(XKCD_COLORS) / sizeof(XKCD_COLORS[0]);

// ==== Lookup function =======================================================
// Normalises input (lowercase, strip leading/trailing spaces) and compares
// against the table.  Returns 0xFFFFFFFF if not found.

static const uint32_t XKCD_NOT_FOUND = 0xFFFFFFFF;

inline uint32_t xkcd_lookup(const String& input) {
    String name = input;
    name.trim();
    name.toLowerCase();
    if (name.length() == 0) return XKCD_NOT_FOUND;

    for (int i = 0; i < NUM_XKCD_COLORS; i++) {
        if (name == XKCD_COLORS[i].name) return XKCD_COLORS[i].rgb;
    }
    return XKCD_NOT_FOUND;
}

// ==== Parse hex RGB string ==================================================
// Accepts "RRGGBB" or "#RRGGBB".  Returns 0xFFFFFFFF on failure.

inline uint32_t parse_hex_rgb(const String& input) {
    String hex = input;
    hex.trim();
    if (hex.startsWith("#")) hex = hex.substring(1);
    if (hex.length() != 6) return XKCD_NOT_FOUND;

    char* end = nullptr;
    uint32_t val = strtoul(hex.c_str(), &end, 16);
    if (end && *end == '\0') return val;
    return XKCD_NOT_FOUND;
}

#endif // BC127_XKCD_COLORS_H
