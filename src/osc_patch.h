// =============================================================================
// osc_patch.h — OscPatch class: a named group of OscMessages with a send task
// =============================================================================
//
// An OscPatch groups one or more OscMessages and drives their transmission
// through a dedicated FreeRTOS task.  The task wakes at the patch's configured
// polling period and sends every enabled message in the group.
//
// OVERRIDE SYSTEM:
//   A patch carries its own ip, port, osc_address, and output bounds (low /
//   high).  For each of these fields the patch can be told to *override* the
//   corresponding field on its child messages.  Override behaviour is
//   controlled per-field via the OverrideFlags struct:
//
//     patch.overrides.ip = true   →  all messages in this patch use the
//                                     patch's IP regardless of their own.
//
//   When an override flag is OFF, each message uses its own value (with the
//   patch providing a fallback if the message's value is not set at all).
//
// ADDRESS COMPOSITION:
//   The address_mode field controls how the patch and message OSC addresses
//   are combined when sending:
//
//     ADR_FALLBACK  (default) — use the message's address; if the message has
//                               no address, fall back to the patch's address.
//     ADR_OVERRIDE            — the patch's address replaces the message's.
//     ADR_PREPEND             — patch address + message address
//                               e.g. "/mixer" + "/fader1" → "/mixer/fader1"
//     ADR_APPEND              — message address + patch address
//                               e.g. "/fader1" + "/mixer" → "/fader1/mixer"
//
// SCALE OVERRIDE:
//   Since all sensor values are normalised to [0, 1], the patch can store its
//   own output bounds (low / high).  When overrides.low or overrides.high is
//   true, the patch's bound replaces the message's bound at send time.  This
//   lets you rescale an entire group of messages uniformly — for example,
//   mapping all values in a patch to [0, 255] for a DMX fixture.
//
// TASK LIFECYCLE:
...
enum AddressMode : uint8_t {
    ADR_FALLBACK = 0,   // message's own address, patch as fallback (default)
    ADR_OVERRIDE = 1,   // patch address replaces message address entirely
    ADR_PREPEND  = 2,   // patch.adr + msg.adr  (patch comes first)
    ADR_APPEND   = 3    // msg.adr + patch.adr  (message comes first)
};
...
// ---------------------------------------------------------------------------
// OverrideFlags — which message fields the patch forcibly replaces
// ---------------------------------------------------------------------------
...
public:
    ...
    // Override flags: when true, the patch's value replaces the message's.
    OverrideFlags  overrides;

    // Indices into OscRegistry::messages[] for every message in this patch.
    ...
    explicit OscPatch(const String& set_name)
        : name(set_name), ip(0, 0, 0, 0), port(0)
    {
        exist.name = true;
    }

    // Construct a patch from an OscMessage (copies shared fields).
    explicit OscPatch(const OscMessage& msg)
    ...
    /// Add a message index to this patch.  No-op if already present or full.
    ...
    /// Remove a message index from this patch.  No-op if not present.
    ...
    /// Check whether a message index is in this patch.
    ...
    /// Build a human-readable summary of this patch (for list/info replies).
